...
class FlxSprite extends FlxObject
{
    ...
    ?key:String
		...
		// Handle string paths first (e.g. "assets/images/foo.png")
		if (Std.isOfType(graphic, String)) {
			var path:String = cast graphic;

			// Try to load via OpenFL Assets so Polymod overrides apply
			if (openfl.Assets.exists(path)) {
				...
				// Optionally: fallback to "assets/images/" if not found
				var altPath = "assets/images/" + path;
				if (openfl.Assets.exists(altPath))
					graphic = openfl.Assets.getBitmapData(altPath);
			}
		}

		// === Original FlxSprite.loadGraphic logic ===
